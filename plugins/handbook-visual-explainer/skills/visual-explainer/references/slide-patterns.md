...
**Step 4 — Plan visuals.** Consider where CSS gradients, SVG decorations, or embedded images would strengthen the deck. At minimum, plan the **title slide** background and **one full-bleed slide** visual treatment. Vary these across the sequence.
...
**Background visuals:** Use CSS gradients and SVG decorations for title slides and full-bleed slides. The `.slide__bg--gradient` pattern with bold `linear-gradient` or `radial-gradient` backgrounds produces compelling results. The deck should stand on its own visually — pure structural or data-heavy decks (code reviews, table comparisons) may not need elaborate backgrounds.
